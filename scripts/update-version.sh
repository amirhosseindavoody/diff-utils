#!/usr/bin/env bash
# Bump project version (Cargo semver) in Cargo.toml, pixi.toml, and recipe.yaml.
#
# Semver stored in files:  YYYY.M.D+N   (e.g. 2026.6.30+0)
# Human / calver meaning: YYYY.MM.DD.N (e.g. 2026.06.30.0)
#
# N is 0 on the first bump of a calendar day; further runs the same day increment N.
# Cargo requires semver; a fourth dotted segment (2026.06.30.0) is invalid.
...
main() {
  ...
  cargo update -p diff-utils -p diff-utils-core --quiet 2>/dev/null || true
